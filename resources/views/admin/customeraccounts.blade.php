...
@section('admincontent')
    ...
    <div class="bg-gray-800 my-10 p-6 rounded-xl">
        <h1 class="text-gray-300 font-bold text-2xl">Existing Customers Account</h1>
        <p class="text-gray-500">Total approved customers: {{ $totalcust }}</p>
        @if ($errors->any())
        ...
        @if (count($customers) > 0)
            <form action="" method="POST" enctype="multipart/form-data" class=" mt-10">
                @csrf
                {{-- giving a hidden data as customer because customer and customer approval page are using the same function approvalprocess in approvalcontroller.php --}}
                ...
                <table class="w-full text-center text-gray-300 table-auto text-sm md:text-md">
                    ...
                    <tr class="text-sm md:text-md lg:text-lg bg-gray-900 border-b border-gray-100">
                        ...
                        <th class="border-l border-gray-600">CustomerId</th>
                        ...
                        <tr class=" border-b border-gray-500 bg-slate-700">
                            ...
                            <td class="border-l border-gray-400">
                                <a class="text-blue-400 underline hover:text-blue-500"
                                    href="{{ route('showIndividualCustomerDetails', $customer->custId) }}">{{ $customer->custId }}
                                </a>
                            </td>
                            ...
                            {{-- more details in individualCustomerDetails.php --}}
                            @if ($customer->verification_file and $customer->kyc_rejection === null)
                                <td class="border-l border-gray-400 bg-blue-100 text-blue-800">
                                    <i class="bi bi-hourglass-split"></i>
                                </td>

                                {{-- if customer has not uploaded file then display this table data --}}
                            @elseif($customer->verification_file === null and $customer->kyc_rejection === null)
                                <td class="bg-red-100 border-l border-gray-400">
                                    <p class="bg-red-100 text-red-800">--No File--</p>
                                </td>
                            @elseif($customer->kyc_rejection === 1)
                                <td class="bg-red-100 border-l border-gray-400">
                                    <i class="bi bi-file-earmark-x-fill text-red-700"></i>
                                </td>
                            @elseif($customer->kyc_rejection === 0)
                            ...
                            {{-- 5th td --}}
                            <td class="border-l border-gray-400">{{ $customer->name }}</td>

                            {{-- 6th td --}}
                            <td class="border-l border-gray-400">{{ $customer->email }}</td>
                            <td class="border-l border-gray-400">{{ $customer->phone }}</td>
                        </tr>
                    @endforeach
        ...
        @else
            <p class="text-center rounded-md py-2 mt-10 text-md bg-gray-600">No customers are approved !</p>
        @endif
    </div>
@endsection
