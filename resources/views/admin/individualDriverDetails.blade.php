    ...
    <div class="bg-gray-800 my-10 p-6 rounded-xl">
        ...
        <div class="flex flex-col lg:flex-row justify-center items-center lg:items-start">
            ...
            <div class="lg:border-r-4 border-gray-500 lg:pr-10 border-b-2 lg:border-b-0 w-full lg:w-fit pb-10 lg:pb-0">
                <h1 class="font-bold text-2xl underline text-center lg:text-left">Driver Personal Details</h1>

                {{-- @foreach ($driverDetails as $c) --}}
                <table class="border border-gray-200 mt-10 mx-auto">
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="p-2 border-l border-gray-200 bg-gray-700 flex justify-center">
                            <img src="{{ asset('uploads/' . $driverDetails->profile_pic) }}" alt="profilePic"
                                class="h-20 rounded-xl">
                        </td>
                    </tr>
                    <tr class="border-b border-gray-200">
                        <td class="p-2 font-bold">Driver Name</td>
                        <td class="p-2 border-l border-gray-200 bg-gray-700">{{ $driverDetails->name }}</td>
                    </tr>
                    <tr class="border-b border-gray-200">
                        <td class="p-2 font-bold">Driver Id</td>
                        <td class="p-2 border-l border-gray-200 bg-gray-700">{{ $driverDetails->driverId }}</td>
                    </tr>
                    <tr class="border-b border-gray-200">
                        <td class="p-2 font-bold">Email</td>
                        <td class="p-2 border-l border-gray-200 bg-gray-700">{{ $driverDetails->email }}</td>
                    </tr>
                    <tr class="border-b border-gray-200">
                        <td class="p-2 font-bold">Phone</td>
                        <td class="p-2 border-l border-gray-200 bg-gray-700">{{ $driverDetails->phone }}</td>
                    </tr>
                </table>
                {{-- @endforeach --}}
            </div>
            ...
            <div class="lg:pl-10 pt-10 lg:pt-0">
                ...
                <h1 class="font-bold text-2xl underline text-center lg:text-left">Edit the Driver Details</h1>


                @if (session('driver_details_updated'))
                    <p class="text-green-500">updated</p>
                @endif

                <form action="{{ route('adminUpdateDriverDetails', $driverDetails->driverId) }}" class="mt-10"
                    enctype="multipart/form-data" method="POST">
                    @csrf
                    <p class="mt-3 justify-between flex items-center">
                        Update Driver Name: <input type="text" placeholder="Enter new Name" name="updatedName"
                    ...
                    <p class="mt-3 justify-between flex items-center">
                        Update Driver Email:<input type="email" placeholder="Enter new Email" name="updatedEmail"
                            class="bg-gray-800 ml-2 text-white placeholder-gray-400 rounded-md border-2 border-gray-600 focus:bg-gray-900 focus:border-blue-500">
                    </p>
                    <div class="text-red-500 text-sm">{{ $errors->first('updatedEmail') }}</div>

                    <p class="mt-3 justify-between flex items-center">
                        Update Driver Phone:<input type="text" placeholder="Enter new Phone" name="updatedPhone"
                            class="bg-gray-800 ml-2 text-white placeholder-gray-400 rounded-md border-2 border-gray-600 focus:bg-gray-900 focus:border-blue-500">
                    </p>
        ...
        <div>
            ...
            {{-- this condition works if Driver has uploaded the kyc file for verification else no need to go in this statement --}}
            {{-- kyc_verification is null in 2 condition, if the user has not uploaded the file, and after user uploads the file.
            if we just check only null value in kyc_rejection field for particular Driver without checking if the kyc file has been uploaded or not, this will not work because weather the Drivers uploads/doesn't uploads that field is going to be null anyway by default as soon as Driver account is created or approved by admin. 
            ...
            so we need to check if it is null after uploading the kyc file by Driver. so either use if($driverDetails->verification_file and $driverDetails->kyc_rejection === null) or put the if($driverDetails->verification_file) and then put other conditions in this so that code enters in this condition only if the Driver has uploaded kyc file.
            after that, the kyc will eiter be approved or rejected. then we can just check the kyc_rejection is 0 or 1. 0=not rejected, 1=rejected
             --}}
            @if ($driverDetails->verification_file)
                @if ($driverDetails->kyc_rejection === null)
                ...
                @elseif($driverDetails->kyc_rejection === 0)
                    <p class="mb-3">User Verification Status:
                        <span class="p-1 text-sm rounded bg-green-50 text-green-900">
                            KYC Verified <i class="bi bi-check2-circle"></i>
                        </span>
                    </p>
                @elseif($driverDetails->kyc_rejection === 1)
                    <p class="mb-3">User Verification Status:
                        <span class="p-1 text-sm rounded bg-red-100 text-red-700">
                ...
                <div class="bg-gray-600 w-fit p-1 rounded flex items-center flex-col mt-10">
                    <p class="text-center mb-2 text-black bg-gray-200 rounded p-1">{{ $driverDetails->verification_type }}
                    </p>
                    <img src="{{ asset('uploads/' . $driverDetails->verification_file) }}" alt="" class="w-1/2">
                </div>

                <p class="mt-4 flex flex-wrap gap-3">
                    <a href="{{ route('AdmindriverKycApprove', $driverDetails->driverId) }}"
                        class="bg-blue-500 w-fit p-3 rounded">Approve</a>
                    <a href="{{ route('AdmindriverKycReject', $driverDetails->driverId) }}"
