...
@section('drivercontent')
    ...
    <div class="bg-gray-800 my-10 p-6 rounded-xl text-white">
        <h1 class="text-white font-bold text-3xl">Driver Profile</h1>
        ...
        {{-- if the currently authenticated driver column verification_file is empty --}}
        @if (!$currentDriverDetails->verification_file)
        ...
        {{-- if the driver uploads file, accepted, rejected or verification is pending, show this message. --}}
        @if (
            $currentDriverDetails->verification_status == 0 and
                $currentDriverDetails->kyc_rejection === null and
                $currentDriverDetails->verification_file)
        ...
        @elseif($currentDriverDetails->verification_status == 0 and $currentDriverDetails->kyc_rejection === 1)
            ...
            {{-- @elseif($currentDriverDetails->verification_status == 1 and $currentDriverDetails->kyc_rejection === 0)
            <p class="bg-green-50 text-green-800 py-2 px-3 rounded-r border-l-4 border-green-600 mt-2">
                KYC Verified.
            </p> --}}
        @endif
        ...
        @if (session('driverDetailsUpdated'))
            <p class="text-green-800 bg-green-50 py-1 px-3 rounded-r border-l-4 border-green-500 mt-2">Username updated
                successfully.</p>
        @endif

        @if (session('driver_verification_file_deleted'))
            <p class="bg-green-50 mt-2 border-l-4 border-green-800 py-2 px-3 text-green-800">KYC file deleted successfully.
            </p>
        @endif
        ...
        <div class="flex flex-col lg:w-1/2 w-full mx-auto bg-slate-700 p-3 rounded mt-10">
            <h1 class="text-2xl font-semibold mb-5">Driver Details</h1>

            @if ($currentDriverDetails->profile_pic)
                ...
                <div class="flex flex-col items-center w-fit gap-2 border-2 border-yellow-600 rounded">
                    <img src="{{ asset('uploads/' . $currentDriverDetails->profile_pic) }}" alt="testing"
                        class="rounded-lg border-2 border-green-500 w-20 mt-5 mx-3">
                    <hr class="border-yellow-500 border w-full text-yellow-500">
                    <a href="{{ route('delete_driver_profile_pic') }}" class="mb-2"><i
            ...
            @else
                <div class="mt-5">
                    ...
                    <form action="{{ route('upload_driver_profile_pic') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input name='driver_profile_pic'
            ...
            <form class="" action="{{ route('UpdateDriverDetails') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <p class="justify-between flex items-center">
                    Driver Id: <input disabled type="text" value="{{ $currentDriverDetails->driverId }}"
                        class="bg-gray-600 text-gray-400 placeholder-white rounded-md bg-opacity-50">
                </p>
                <p class="mt-3 justify-between flex items-center">
                    Wallet Id: <input disabled type="text" value="{{ $currentDriverDetails->driverwallet->wallet_id }}"
                        class="bg-gray-600 text-gray-400 placeholder-white rounded-md bg-opacity-50" />


                <p class="mt-3 justify-between flex items-center">
                    User Name: <input disabled type="text" value="{{ $currentDriverDetails->name }}"
                        class="bg-gray-600 text-gray-400 placeholder-white rounded-md bg-opacity-50 ">
                </p>
                ...
                <p class="mt-3 justify-between flex items-center">
                    Current Email: <input disabled type="text" value="{{ $currentDriverDetails->email }}"
                        class="bg-gray-600 text-gray-400 placeholder-white rounded-md bg-opacity-50">
                </p>
                <p class="mt-3 justify-between flex items-center">
                    Current Phone: <input disabled type="text" value="{{ $currentDriverDetails->phone }}"
                        class="bg-gray-600 text-gray-400 placeholder-white rounded-md bg-opacity-50">
                </p>
                <p class="mt-3 justify-between flex items-center">
        ...
        <form action="{{ route('driverDocVerification') }}" method="POST" enctype="multipart/form-data"
            ...
            <h1 class="text-2xl font-semibold mb-5">KYC Verification
                ...
                @if ($currentDriverDetails->verification_status)
            ...
            <div class="flex flex-col">
                ...
                @else
                    ...
                    <div class="bg-gray-500 w-fit flex items-center flex-col p-1 rounded mt-3">
                        <img src="{{ asset('uploads/' . $currentDriverDetails->verification_file) }}" alt=""
                            class="h-24 w-24">
                        <a href="{{ route('driverDeleteVerificationFile') }}" class="mt-2">
                            <i
